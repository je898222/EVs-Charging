...
class StationEnv(AECEnv):
    ...
    def __init__(self, mode = "train", N_piles = 5):
        # self._seed()
        self.mode    = mode
        ...
        self.P_pile = (5+np.random.rand()*3) #(3.5+np.random.rand()*1.5) #(kW)

        self.P_max_station = self.N_piles*self.P_pile
        self.P_max_station = 800

        self.car_type_list = [ "Chevy Volt", "Volkswagen E-Golf", "BMW i3", "Tesla Model S"]#, "Tesla Model S"
    ...
    def reset(self, seed=None, options=None, select_pile = "pile_0", price_data = []):
        ...
        if self.mode == "train":
            ...
            while len(self.execute_Pgrid_price) < 96:
                self.execute_Pgrid_price = np.append(self.execute_Pgrid_price, [1+np.random.rand()*6]*np.random.randint(1,41) ).astype(np.float32) #5
    ...
    def step(self, action):
        ...
        if self.mode != "train":
            self.P_plot_action[now_agent] = charge_power/ self.pile[now_agent].P_max_charge
        # charge_soc ----------
        ...
        if self._agent_selector.is_last():
            print(f'total_power:{sum(self.P_already.values())}')
            print(self.P_already)
